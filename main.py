...
def afficher_graphs(valeur):
    # Exemple de données pour démonstration #####  A ENLEVER   #####
    ...
    draw_plot(gx_inner, x, y1, "Déplacement X")  ##Y1 SERA LE GRAPH DU DePLACEMENT EN FONCTION DE X RENVOYe PAR LA FONCTION DU GROUPE B
    ...
    ax2.scatter(np.sin(x), np.cos(x), c=x, cmap="viridis")  ## np.sin(x) A REMPLACER  par la fonction du groupe C Léonie et Manon
    ...
    ax3.scatter(np.sin(x), np.cos(x), x, c=x, cmap="plasma")  ## np.sin(x) A REMPLACER  par la fonction du groupe C Peyo
